...
The CMake build output lists the modules by name::
    ...
    ...
    Including User C Module(s) from ../../examples/usercmodule/usermod.cmake
    Found User C Module(s): usermod_cexample, usermod_cppexample
    ...


Note that the ``usermod.cmake`` files define ``DMODULE_<name>_ENABLED=1`` automatically.
The top-level ``usermod.cmake`` can be used to control which modules are enabled.
